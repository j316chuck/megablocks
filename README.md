...
Installing `megablocks[te]` enables dMoE computation with TransformerEngine. This feature is enabled by setting the `mlp_impl` argument to `te`. This enables forward and backward passes for the MoE to be done in fp8 which has been shown to speed up performance.
